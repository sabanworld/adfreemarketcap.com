...
|---------|------------------|------|
| Sitemap | `/sitemap.xml` (`sitemap` route) | Home, DexScan, Why ad-free, every legal page, every coin slug, DexScan pairs (up to 500 by volume), and synced Dex tokens (up to 500); `lastmod` from sync timestamps when present |
...
4. **HTTP error pages** (`resources/views/errors/`) use `SeoService::forErrorPage()` with `noindex,nofollow`. Do not add them to the sitemap.
5. When renaming routes or slug patterns, update `SeoService`, sitemap tests in `tests/Feature/SeoTest.php`, and this doc in the same change.
6. Prefer meaningful `alt` text on public images (coin logos use the coin name).
